# 📚 Curso de Análisis de Algoritmos y Estrategias de Progreso

Este repositorio contiene ejercicios y prácticas desarrolladas durante el curso de **Análisis de Algoritmos y Estrategias de Progreso**.

## 🎯 Objetivo

El objetivo de este repositorio es documentar mi progreso en el curso, reforzando conceptos fundamentales mediante la resolución de ejercicios prácticos.

## 🧠 Contenido

- Ejercicios básicos de algoritmos
- Prácticas de lógica de programación
- Implementaciones progresivas según el avance del curso

## 🚀 Tecnologías utilizadas

- Lenguaje(s): *(puedes completar aquí, por ejemplo: Java, Python, etc.)*
- IDE: *(opcional, por ejemplo: IntelliJ, VS Code, etc.)*

## 📁 Estructura del repositorio
/
├── Semana##/ (irá aumentando con el avance en el curso Ej: Semana04, Semana05)
├── Proyecto/
├── Documentos/ (ignorado en Git)
└── README.md


## 📌 Notas

- La carpeta `Documentos` contiene material adicional y no forma parte del seguimiento del repositorio.
- Este repositorio será actualizado constantemente conforme avance en el curso.

## ✍️ Autor

- Sergio Diaz Mantilla

---